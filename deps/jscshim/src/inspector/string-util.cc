...
#include "auto-generated/Protocol.h"
...
String16 toProtocolString(v8::Local<v8::String> value) {
  ...
  value->Write(v8::Isolate::GetCurrent(), reinterpret_cast<uint16_t*>(buffer.get()), 0, value->Length());
...
// static
// jscshim: Based on WTF::String's charactersToDouble (toDoubleType)
// TODO jscshim: WTF's parseDouble doesn't seem to handle hex\octal\binary
double StringUtil::toDouble(const char* s, size_t len, bool* isOk) {
  ...
  *isOk = !std::isnan(result);
  ...
  while (leadingSpacesLength < len && isASCIISpace(s[leadingSpacesLength]))
    ++leadingSpacesLength;

  double number = parseDouble((const LChar *)s + leadingSpacesLength, len - leadingSpacesLength, parsedLength);
  if (!parsedLength) {
    *isOk = false;
    return 0.0;
  }
