...
public:
	...
	{
		// TODO: Initialize all values with WTF::OrdinalNumber::beforeFirst()?
...
public:
	...
	// Taken from JSC::StackFrame
	bool hasBytecodeOffset() const { return (UINT_MAX != m_bytecodeOffset) && !m_isWasmFrame; }
...
public:
	...
	static JSCStackTrace captureCurrentJSStackTrace(JSC::ExecState * exec, size_t frameLimit, size_t framesToSkip = 0);
...
private:
	JSCStackTrace(WTF::Vector<JSCStackFrame>& frames) : m_frames(WTFMove(frames))
	{
	}
};
