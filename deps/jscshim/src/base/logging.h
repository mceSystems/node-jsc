...
/* jscshim: We'll replace WTF's FATAL macro with one that doesn't use the file name\line number\function name
 * on relase builds, like v8's macro */
#undef FATAL
...
#else
	#define FATAL(...) do { \
		WTFReportFatalError("", 0, "", __VA_ARGS__); \
		CRASH(); \
	} while (0)
#endif
