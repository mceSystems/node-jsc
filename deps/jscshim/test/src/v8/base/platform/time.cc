...
Time Time::Now() {
  ...
  DCHECK_EQ(0, result);
  // jscshim: TODO: v8 defines it in base\macros.h, but WTF also defines a "USE" macro (for different purposes)
  // USE(result);
...
bool ThreadTicks::IsSupportedWin() {
  static bool is_supported = base::V8_CPU().has_non_stop_time_stamp_counter() &&
                             !IsQPCReliable();
  return is_supported;
}
